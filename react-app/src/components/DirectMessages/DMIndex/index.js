...
export default function DMIndex({ theme }) {
  ...
  if (isLoaded && friend && servers && theme) {
    ...
    const deleteHandlerCurr = (e) => {
      ...
      const msg = messages.filter((el) => el.message !== e.target.dataset.msg);
      setMessages(msg);
      ...
      user && (
        <div className="cms-container dms">
          <div className="cms-ch-name" id={theme}>
            <div className="cms-friend-name">{`@${friend.username}`}</div>
            <div className="row overfl-wid">
              <div className="dm-aka">{`AKA`}</div>
              <div className="dm-row">
                {[...otherNicknames].slice(0, 6).map((el, i) => {
                  if (el) {
                    return (
                      <span
              ...
              {dms.length > 0 ? (
                dms.map((message, i) => (
                  <div id={theme} className="row justify" key={message.id}>
                    {Number(message.sender_id) === Number(senderId) ? (
                      <div className="img-link" data-id={message.id}>
                        <img
                      ...
                      <div className="msg-ct">
                        <div className="cms-msg-header" id={theme}>
                          {Number(message.sender_id) === Number(user.id) ? (
                            ...
                            <div id={theme} className="dms-member">
                              {`${friend.username}`}
                            </div>
                          )}
                          ...
                          {Number(user.id) === Number(message.sender_id) ? (
                            <div id={theme} className="cms-options absolute-op">
                              <div
                                className="cms-edit"
                                data-id={message.id}
                                onClick={handleEdit}
                      ...
                      {Number(message.id) === Number(edit) ? (
                        ...
                        <div className="msg-ct">
                          <div className="cms-msg-header">
                            ...
                            {Number(senderId) === Number(message.sender_id) ? (
                              <div
                                id={theme}
